...
df1 = df %>% 
  ...
  select(CASENUM, VE_TOTAL.1, VE_FORMS.1, PVH_INVL.1, PERMVIT.1, 
         ...
         NUMOCCS.1, UNITTYPE.1, HIT_RUN.1, BODY_TYP.1, MOD_YEAR.1, TOW_VEH.1, 
         ...
         NUMOCCS.2, UNITTYPE.2, HIT_RUN.2, BODY_TYP.2, MOD_YEAR.2, TOW_VEH.2, 
...
df2 = df1 %>%
  ...
  ## Person level
  filter(AGE.1 < 120) %>% mutate(AGE1 = cut(AGE.1, breaks = c(0,25,40,60,100), labels = c("Young","Aldut","Middle","Old"), include.lowest = TRUE)) %>% 
  filter(AGE.2 < 120) %>% mutate(AGE2 = cut(AGE.2, breaks = c(0,25,40,60,100), labels = c("Young","Aldut","Middle","Old"), include.lowest = TRUE))  %>% 
