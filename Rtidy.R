...
dfdemo = df3 %>% 
  ...
  pivot_longer(-c(CASENUM, WEATHHER),
               ...
               names_sep = "\\.",
               values_drop_na = TRUE)
write_csv(dfdemo, "/Users/cpf/Documents/paper/R_programming/Rprocess/mydata/dfdemo.csv")
...
df3 = df2 %>% 
  select(CASENUM,VEH_NO.1,VEH_NO.2,REST_USE.1,REST_USE.2,INJ_SEV.1,INJ_SEV.2,HOUR.1,  AGE1.1,AGE.1, DRINKING.1, DRUGS.1, AIR_BAG.1, EJECTION.1,ROLLOVER.1,
         AGE1.2,AGE.2 ,DRINKING.2, DRUGS.2, AIR_BAG.2, EJECTION.2, ROLLOVER.2, MANCOLL, LGT, WEATHHER, WEATHER2) %>% 
  drop_na()  
...
fit.mvf6 <- mvord(formula = MMO2(REST_USE.1, REST_USE.2, INJ_SEV.1,INJ_SEV.2) ~ 0 + AGE1.1+ DRINKING.1 + DRUGS.1 +AIR_BAG.1 + EJECTION.1 + ROLLOVER.1 + AGE1.2 + DRINKING.2+  DRUGS.2  + AIR_BAG.2+ EJECTION.2 + ROLLOVER.2 + MANCOLL + LGT  + WEATHHER, 
                 threshold.constraints = c(1,2,3,4),
                 link = mvprobit(),data = as.data.frame(df5[1][[1]][1:1000,]))
